...
export const FloatingDock = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full flex justify-center items-center z-50 pointer-events-none px-4">
      ...
      >
        <div className={cn(
          "bg-black/20 backdrop-blur-xl border border-white/10 px-3 py-2 rounded-2xl flex items-center gap-2.5 shadow-2xl mx-auto",
          isMobile && "w-[95%] overflow-x-auto justify-between scrollbar-none"
        )}>
          {items.map((item, idx) => (
            <div
              key={idx}
            ...
            >
              {hoveredIndex === idx && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute -top-11 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-black/80 rounded-lg text-white text-xs font-medium shadow-lg backdrop-blur-sm border border-white/10 whitespace-nowrap z-50"
                >
              ...
              <motion.div
                ...
                whileTap={{ scale: 0.92 }}
                className={`relative ${isMobile ? 'w-10 h-10' : 'w-11 h-11'} flex items-center justify-center rounded-full ${
